...
namespace VRC6Dlg
{
	...
	BOOL __stdcall	 DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_COMMAND:
		{
			...
			{
				if (wmId >= IDC_MIX_1 && wmId <= IDC_MIX_3)
				{
			...
			{
				if (wmId >= IDC_VOLBOX_1 && wmId <= IDC_VOLBOX_3)
				{
					CONFIG_CHANGE_NOTIFY();
				}
				else if (wmId >= IDC_PANBOX_1 && wmId <= IDC_PANBOX_3)
				{
					CONFIG_CHANGE_NOTIFY();
				}
			}
		...
		case WM_HSCROLL:
		{
			...
			for (i = 0; i < 3; i++)
			{
				if (reinterpret_cast<HWND>(lParam) == hVol[i])
				{
					...
					//nsfCore.SetChannelOptions(i + 5, -1, j, 1000, -1);
					CONFIG_CHANGE_NOTIFY();
				}
				if (reinterpret_cast<HWND>(lParam) == hPan[i])
				{
					...
					//nsfCore.SetChannelOptions(i + 5, -1, -1, j, -1);
					CONFIG_CHANGE_NOTIFY();
				}
			}
		}
