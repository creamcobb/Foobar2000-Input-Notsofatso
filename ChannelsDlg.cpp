	...
	BOOL __stdcall	 DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_COMMAND:
		{
			...
			if (wmId >= IDC_MIX_1 && wmId <= IDC_MIX_6)
			{
				...
				{
					//for (int i = 0; i < 5; i++)
						//nsfCore.SetChannelOptions(i, IsDlgButtonChecked(hDlg, IDC_MIX_1 + i), -1, 1000, -1);
					//nsfCore.SetChannelOptions(28, IsDlgButtonChecked(hDlg, IDC_MIX_6), -1, 1000, -1);
					CONFIG_CHANGE_NOTIFY();
				}
			}
			else if (wmId >= IDC_VOLBOX_1 && wmId <= IDC_VOLBOX_6 && wmEvent == EN_UPDATE)
			{
				CONFIG_CHANGE_NOTIFY();
			}
			else if (wmId >= IDC_PANBOX_1 && wmId <= IDC_PANBOX_6 && wmEvent == EN_UPDATE)
			{
				CONFIG_CHANGE_NOTIFY();
			}
			else
		...
		case WM_HSCROLL:
		{
			...
			for (i = 0; i < 6; i++)
			{
				if (reinterpret_cast<HWND>(lParam) == hVol[i])
				{
					...
					//nsfCore.SetChannelOptions(((i == 5) ? 28 : i), -1, j, 1000, -1);
					CONFIG_CHANGE_NOTIFY();
				}
				if (reinterpret_cast<HWND>(lParam) == hPan[i])
				{
					...
					//nsfCore.SetChannelOptions(((i == 5) ? 28 : i), -1, -1, j, -1);
					CONFIG_CHANGE_NOTIFY();
				}
			}
		}
